...
- **shadcn/ui v4 (Radix Nova)** — `components.json` style `"radix-nova"`, 45 components in `components/ui/`, icon library `lucide-react`.
...
4. **Notifications** — 37 event types (1:1 between the Prisma `NotificationType` enum and the TS union at `lib/notifications/types.ts`). Adding a new event requires touching 4 files (see AGENTS.md §Notifications). SSE at `/api/notifications/stream` with polling fallback.
